...
import type { TaskCommentRecord } from "@/features/tasks/action-service";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import type { VerifiedUser } from "@/lib/auth/types";
import type { TaskCommentView } from "@/features/tasks/types";
import { FormEvent, useState } from "react";

type AddCommentResult =
  | { ok: true; data: TaskCommentRecord }
...
export function TaskComments({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await addCommentAction(taskId, trimmedBody);
    setIsSaving(false);
  ...
  return (
    <section className="rounded-xl border bg-card p-5">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-medium">留言</h2>
      ...
      <div className="space-y-4">
        ...
        {comments.length === 0 ? (
          <p className="py-3 text-center text-sm text-muted-foreground">还没有留言</p>
        ) : null}
      </div>

      <form onSubmit={handleSubmit} className="mt-5 grid gap-3 border-t pt-4">
        <label htmlFor="task-comment" className="text-sm font-medium">写留言</label>
        <textarea
          id="task-comment"
          value={body}
